...
(defun project-check-status (source-directory verbose)
  (handler-case
      (progn
        ...
        (let ((failures
                (run-system-tests (local-system-names source-directory))))
          (when failures
...
(defun run-project-checks ()
  (let* ((verbose (verbose-checks-p)))
    (uiop:quit
     (run-function
      (lambda () (project-check-status (get-source-directory) verbose))
      :verbose verbose))))
